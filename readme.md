The repository contains the companion codes to the paper `On the Dynamics of Transporting Rolling Cylinders` that is in preperation for submittion to Nonlinear Dynamics in May 2024.

# Frobenius Integrability Criteria
The folder `frobenius-integrability-criteria` implementation of the Frobenius Criteria in Matlab symbolically and with a numerical example.
...
1. Create an input file that includes in order the cylinder weight, radius, height, angle of inclination with the vertical, and the initial coordinates of the centers of mass of both cylinders following the manner of `generalized-alpha-simulation/inputs-movie.txt`
